# 🏦 Quant-X: 교육용 멀티 에이전트 금융 리서치 시스템

> **Fastcampus 금융 AI Agent 구축 튜토리얼을 위한 멀티에이전트 협업 투자 딥리서치 시스템**

## 📋 프로젝트 개요

Quant-X는 **멀티 에이전트 협업(Multi-Agent Collaboration)**을 통한 금융 리서치 시스템입니다. Manager-Worker 패턴을 활용하여 역할별 전문 에이전트가 협업하며, 금융권의 엄격한 보안 규제와 시스템 통제(Governance)를 준수하는 **교육용 AI 에이전트 시스템**입니다.

### 🎯 핵심 철학
**"전문화된 AI 에이전트들이 협업하여, 시스템의 통제 하에 역할별 최적화된 금융 분석을 수행한다."**

### 🏗️ 멀티 에이전트 아키텍처
```
┌─────────────────────────────────────────────────────────────────┐
│                    Frontend (Multi-Tab UI)                     │
│  📋 종합 상황판  │  🕵️ 리서치 센터  │  📈 시장 분석실  │
└─────────────────────────────────────────────────────────────────┘
                                │
┌─────────────────────────────────────────────────────────────────┐
│                      Governance Layer                          │
...
│  ┌─────────────────────────────┼─────────────────────────────┐  │
│  │                             │                             │  │
│  🔍 Research Agent             📈 Market Analyst Agent       │  │
│  ├── 정보 수집 전문가           ├── 시장 데이터 분석가         │  │
│  ├── 사내 검색 우선             ├── 주가 및 지수 분석          │  │
│  ├── 웹 검색 보조               ├── 재무 지표 해석             │  │
│  └── 뉴스/공시 수집             └── 차트 데이터 생성           │  │
│                                                                 │
...
### 🤝 멀티 에이전트 협업
- **Manager-Worker 패턴**: 역할별 전문화된 에이전트 협업
- **실시간 협업 시각화**: UI에서 에이전트 간 작업 과정 실시간 표시
- **중간 산출물 공유**: 에이전트 간 데이터 및 분석 결과 공유
- **Human-in-the-Loop**: 사용자 승인/재작성 요청 인터페이스
...
### 🧠 전문화된 AI 에이전트
- **🔍 Research Agent**: 정보 수집 전문가 (사내 검색 우선, 웹 검색 보조)
- **📈 Market Analyst Agent**: 시장 데이터 분석가 (주가, 지수, 재무 지표)
- **👔 Manager Agent**: 프로젝트 매니저 (협업 조율, 최종 보고서 작성)
- **smolagents 기반**: Python 코드 생성을 통한 논리적 추론

### 📊 Multi-Tab UI 시각화
- **📋 종합 상황판**: 매니저의 종합 브리핑 및 최종 보고서
- **🕵️ 리서치 센터**: 리서처가 수집한 뉴스, 공시 자료 원문 표시
- **📈 시장 분석실**: 애널리스트의 주가 차트 및 재무 지표 시각화
...
│
├── 📁 agents/                   # 멀티 에이전트 실행 레이어
│   ├── 🤖 core.py              # 멀티 에이전트 코어 로직 (Manager-Worker 패턴)
│   └── 🛠️ tools.py             # 도구 함수 (미들웨어 패턴, 시각화 지원)
│
└── 📁 data/                     # 데이터 저장소
    ├── 📁 reports/             # 생성된 리포트
